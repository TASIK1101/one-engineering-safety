...
export default function WorkPermitReopenButton({ permitId }: { permitId: string }) {
  ...
  async function handleReopen() {
    ...
    setError("");
    try {
      const res = await fetch("/api/work-permits/reopen", {
        method: "POST",
  ...
  return (
    <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm">
      <p className="text-amber-800 mb-2 font-medium">
        기존 관리자 전자서명이 모두 무효화되고 재승인이 필요합니다. 진행할까요?
      </p>
